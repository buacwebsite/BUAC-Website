...
const ClubFair = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post("/api/club-fair/submit", {
        Name: form.name,
      ...
      setSubmitStatus({
        type: "success",
        message: "Registration submitted successfully! See you at the fair.",
      });
      setForm(initialForm);

      // Optimistically bump live counter
  ...
  const inputClass =
    "w-full bg-background/80 border border-text-secondary/20 rounded-xl px-4 py-3 text-text-secondary focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent transition-all duration-300";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-text-muted text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
            Meet BUAC at the Club Fair and take your first step into the world
            of adventure. Register below to stay connected.
          </p>
        </div>
        ...
        <motion.div
          ...
          className="mb-20 rounded-3xl border-2 border-accent/30 bg-linear-to-br from-accent/10 via-accent/5 to-transparent p-10 md:p-16 text-center shadow-2xl relative overflow-hidden"
        >
          {/* Decorative glow */}
          ...
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-10">
              <p className="text-text-muted text-lg max-w-2xl mx-auto leading-relaxed">
                Discover your inner explorer and join BUAC on a journey of
                self-discovery! Immerse yourself in thrilling outdoor
                adventures, surrounded by breathtaking landscapes and
                eco-friendly initiatives. Your extraordinary adventure awaits.
              </p>
              <p className="text-text-muted text-lg max-w-2xl mx-auto mt-4">
                Fill out the form below and dive into the excitement and make
            ...
            <motion.form
              ...
              className="bg-background/50 backdrop-blur-sm border border-text-secondary/10 rounded-2xl p-8 shadow-xl space-y-6"
            >
              {submitStatus.type && (
                <div
                  ...
                  <div className="flex gap-2">
                    {bloodDonationOptions.map((opt) => (
                      <button
                        key={opt}
                        type="button"
                        onClick={() => update("bloodDonation", opt)}
                        className={`flex-1 rounded-xl border px-3 py-3 text-sm font-semibold transition-all cursor-pointer ${
                          form.bloodDonation === opt
                            ...
                            : "border-text-secondary/20 bg-background/80 text-text-muted hover:border-accent/50"
                        }`}
                      >
                        {opt}
                      </button>
        ...
        <div className="bg-linear-to-r from-accent/10 via-accent/5 to-accent/10 backdrop-blur-sm border border-accent/20 rounded-2xl p-12 text-center shadow-xl">
          ...
          <p className="text-text-muted text-lg max-w-2xl mx-auto mb-8">
            Feel free to reach out to us. We&apos;re here to help and answer any
            questions you might have about joining BUAC.
          </p>
          <Link
            href="/contact"
