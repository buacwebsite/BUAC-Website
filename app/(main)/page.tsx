...
export default function Home() {
  ...
  // ------ Section order (admin reorderable) ------
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        if (res.data?.order) setSectionOrder(res.data.order);
      } catch (err) {
  ...
  // ------ Section render functions (order-driven) ------

  const renderAboutSection = () => (
    <MotionSection
      key="about"
      className="snap-section relative min-h-screen bg-linear-to-b from-background via-background via-85% md:via-65% to-black py-12 px-6 lg:px-12 font-poppins overflow-hidden"
    >
      {auth && (
        <motion.button
          ...
          className="absolute top-8 right-8 z-10 bg-accent text-white py-2 px-2 md:py-2 md:px-4 mb-2 flex items-center justify-center gap-2 text-sm md:text-base font-medium
          rounded-full border-2 border-accent hover:bg-transparent hover:text-accent transition-all duration-300 cursor-pointer"
          title="Edit Section"
        >
          ...
          {quotes.map((quote, index) => (
            <StaggerItem key={index}>
              <motion.div
                ...
                className="group relative bg-linear-to-br from-accent/10 to-white/30 backdrop-blur-sm border border-text-muted/20 rounded-3xl py-8 px-4 hover:border-accent/70
                transition-all duration-500 shadow-xl overflow-hidden h-full"
              >
              ...
              {objectives.map((objective, index) => (
                <StaggerItem key={index}>
                  <motion.div
                    whileHover={{ scale: 1.05 }}
                    transition={{ duration: 0.25 }}
                    className="group relative bg-linear-to-br from-accent/10 to-text-secondary/5 backdrop-blur-sm border border-accent/30 rounded-2xl p-8 hover:border-accent
                    transition-all duration-500 shadow-xl overflow-hidden h-full"
                  >
  ...
  return (
    <>
      ...
      {auth && orderLoaded && (
        <div className="relative z-20">
          <motion.button
            ...
            className="fixed top-24 left-6 z-40 flex items-center gap-2 rounded-full bg-black/80 border-2 border-accent text-white py-2 px-4 text-sm font-medium shadow-lg
      ...
      {/* AUTH CTA — always fixed at bottom */}
      {!isLoggedIn && (
        <MotionSection className="snap-section relative min-h-[55vh] bg-linear-to-b from-background to-black py-16 px-6 lg:px-12 font-poppins overflow-hidden flex items-center
        justify-center">
          ...
          <div className="relative z-10 max-w-4xl mx-auto text-center">
            ...
            >
              <Link
                href="/register"
                className="inline-flex min-w-[210px] items-center justify-center rounded-full bg-accent px-10 py-4 font-bebasNeue text-2xl tracking-wider text-white shadow-xl
                shadow-accent/25 transition-all duration-300 hover:-translate-y-1 hover:bg-accent/90 hover:shadow-accent/40"
              >
                Create Account
              </Link>
              <Link
                href="/login"
                className="inline-flex min-w-[170px] items-center justify-center rounded-full border-2 border-accent px-10 py-4 font-bebasNeue text-2xl tracking-wider text-accent
                transition-all duration-300 hover:-translate-y-1 hover:bg-accent hover:text-white"
              >
                Sign In
              </Link>
            </motion.div>
      ...
      {/* LOGGED IN CTA — always fixed at bottom */}
      {isLoggedIn && user && (
        <MotionSection className="snap-section relative min-h-[50vh] bg-linear-to-b from-background to-black py-16 px-6 lg:px-12 font-poppins overflow-hidden flex items-center
        justify-center">
          <div className="absolute inset-0 opacity-10">
            <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-accent rounded-full blur-3xl" />
          </div>
          ...
          <div className="relative z-10 max-w-4xl mx-auto text-center">
            ...
            >
              <Link
                href="/tours"
                className="inline-flex min-w-[170px] items-center justify-center rounded-full bg-accent px-8 py-4 font-bebasNeue text-xl tracking-wider text-white shadow-xl
                shadow-accent/25 transition-all duration-300 hover:-translate-y-1 hover:bg-accent/90"
              >
                Explore Tours
              </Link>
              <Link
                href="/activities"
                className="inline-flex min-w-[180px] items-center justify-center rounded-full border-2 border-accent px-8 py-4 font-bebasNeue text-xl tracking-wider text-accent
                transition-all duration-300 hover:-translate-y-1 hover:bg-accent hover:text-white"
              >
                View Activities
              </Link>
              <button
                type="button"
                onClick={logout}
                className="inline-flex min-w-[150px] items-center justify-center rounded-full border-2 border-red-500 px-8 py-4 font-bebasNeue text-xl tracking-wider text-red-400
                transition-all duration-300 hover:-translate-y-1 hover:bg-red-500 hover:text-white"
              >
                Sign Out
              </button>
            </motion.div>
